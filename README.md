...
The Lean files hold 40 statements. **16 are accepted by the Lean kernel** and 24 are not:
...
Every column below is **computed**, and each carries a different epistemic weight. The status is not typed here: `npm run derivation` corrupts the line that produces each column and asks the the 32 seals in src/verification plus computeVortexInvariantsHold in src/0 — 33 predicates that state laws rather than conventions whether anything broke. **derived** means a law fell; **defined** means none did. That is a lower bound — a perturbation respecting a symmetry can leave every law standing — so a defined column is one nothing has been shown to force, not one proven free.

| column | source | status |
| --- | --- | --- |
| mirror `M(d)` | `throughVoid` | **derived** — perturbing it breaks 3 law(s) (`doubling_avoids_the_triad`, `a432_constants_do_not_drift_from_the_kernel`, `merkaba_is_two_mirrored_tetrahedra`); `1 − n mod 9` on the nine digits `1..9`, reading 9 as the residue 0 — the void is a tenth point outside that (ℤ/9ℤ) (Lean `mirror_is_affine_only_off_the_void`) |
| polarity, Δ°, bearing | `decodeVortexDashAngles()` | **derived** — perturbing it breaks 1 law(s) (`computeVortexInvariantsHold`); `\` = −60°, `/` = +60°, weighted bearing closes at 0 |
| gateway | `vortexStrokeGateways()` | **derived** — perturbing it breaks 1 law(s) (`computeVortexInvariantsHold`); polarity reversals `[8, 3, 9, 0]` |
...
| Hz | `frequencyForDigit` | **defined** — 2 perturbation(s) tried, no law objected; `432·d/12` on the axis, throws off-axis |
...
Package: **`zeropoint-node@1.5.0`** · owner `ceccec` · git tag `v1.5.0`
