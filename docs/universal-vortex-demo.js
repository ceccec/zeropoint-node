...
// Import the universal vortex system (simulated for demo)
const {
  createUniversalVortexSystem,
  detectEnvironment,
  getVortexProperties,
  discoverStaticContent,
  serveStaticContent,
  navigateStaticContentByVortex,
  getContentDiscoverySummary,
  digitArchetypes
} = require('./index.ts');
...
class UniversalVortexDemo {
  ...
  demonstrateUniversalSystem() {
    ...
    directories.forEach(dir => {
      ...
      this.system = createUniversalVortexSystem(dir);
      ...
      Object.entries(distribution).forEach(([digit, info]) => {
        const archetype = digitArchetypes[parseInt(digit)];
  ...
  demonstrateVortexProperties() {
    ...
    exampleFiles.forEach(file => {
      const properties = getVortexProperties(file);
  ...
  demonstrateContentServing() {
    ...
    examplePaths.forEach(path => {
      const served = serveStaticContent(path);
  ...
  demonstrateVortexNavigation() {
    ...
    const currentContent = getStaticContent(examplePath);
    ...
    if (currentContent) {
      ...
      const vortexAItems = navigateStaticContentByVortex(examplePath, 'vortexA');
      ...
      const vortexBItems = navigateStaticContentByVortex(examplePath, 'vortexB');
