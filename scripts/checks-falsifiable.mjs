...
const READS_SOURCE = {
  ...
  'quantum:doc:check': 'regenerates docs/QUANTUM_COMPUTER.md from the records that impostors, query:cost, capacity and shor:exhaustive write, and fails if the committed page differs. Written because this package carried four representations of a quantum state, six exact modules and three MCP tools, and a person who installed it could find none of them — the README describes digit arithmetic and the quantum surface was reachable only by reading the source. EVERY NUMBER ON THE PAGE IS READ FROM A RECORD: the count of algorithms identified by method, the oracle-call table, the state-vector-against-tableau timings, the Shor sweep and the list of what was NOT measured. A hand-written copy of a measurement is the defect this repository keeps finding, most recently an MCP server reporting its version as a literal five minors stale. Falsified by editing the page; it also refuses to run when any of the four records is absent, since a page that states measurements must not invent them, and refuses when it reads zero quantum tools from the server.',
  'a432:orbit:check': 'requires every prefix of the kernel\'s doubling orbit to have a module at its address, requires each of those modules to LOAD ITS OWN ADDRESS, and requires the loader that reaches them to match the orbit. a432:names found three addresses with nothing at them — 1.2.4, 1.2.4.8 and 1.2.4.8.7 — so the space held the seed, the first step, and then jumped to the closed six-cycle with the middle of the orbit unaddressed. The modules are GENERATED from the shortest existing member rather than hand-copied, since the family is one file with a length changed and copying it again is another place for the next edit to miss. The orbit is read from VORTEX_ORBIT, so adding a digit adds an address and nothing here lists the sequence. The second law is the one that matters: a name here is a handle carrying its load implicitly, so a432.1.2.4 is not named after 1-2-4, it IS that sequence, and a module at the right address holding the wrong one is a broken handle nothing else would notice. The seven modules all export the same names, so a432.index.ts cannot re-export them and every one was reachable from no static entry — a432.orbit.ts maps address to module with LITERAL paths, because a computed specifier loads the same module while staying invisible to every tool that reads the import graph. Falsified by deleting an address, by changing a module to load a different sequence than its name, or by editing the generated loader; it also refuses when the kernel exports an empty orbit and refuses to invent a template when no member exists to take one from.',
  'a432:names:check': 'audits the a432 filename space as an ADDRESS space, because these names are handles rather than descriptions — a name carries its load implicitly and you unpack it at will, the way a hexbit does. I first wrote them off as payloadless labels; they are not, and the audit loads every one of them. Two families are decidable rather than a matter of taste. THE ORBIT: digit-only names address the kernel\'s doubling sequence, read from VORTEX_ORBIT rather than listed here, so the seven prefixes are the addresses that exist by construction — four are files and three are not, and those three are named. THE LATTICE: every handle folds to six bits, one per digit derived from it, which is the same 64-cell space hexbit:bench walks; 226 handles occupy 63 of the 64 cells and exactly one address has nothing at it. Spelling drift is the third family and the crudest: two segments reducing to one stem are one coordinate spelled twice. The ceilings only fall — orbit gaps, empty cells and drifted stems may each go down and never up. Falsified by adding a file whose handle lands on an already-crowded cell while an orbit prefix stays empty, or by renaming one segment to a second spelling of another; it also refuses when it reads zero files, since an empty directory would otherwise report a complete address space.',
  'capacity:check': 'measures FIVE axes of simulation cost and names five more it does not. query:cost counts oracle calls on two functions and finds no advantage; that is true and narrow, and it was generalised in prose to "the more quantum the work is, the slower it runs", which is false. Gottesman-Knill: a Clifford circuit is classically simulable in POLYNOMIAL time, so the exponential cost of exact.ts and clifford-t.ts belongs to the STATE-VECTOR REPRESENTATION and not to the work being quantum — 14730ms against under a millisecond at 20 qubits, and 1000 qubits on a tableau in 174ms where a state vector needs 2^1000 amplitudes. It also measures WHERE the hardness is, which was previously cited rather than computed: T = a*I + b*S with both coefficients exact in Z[zeta8] over 2, so a circuit with t T-gates is an exact sum of 2^t CLIFFORD circuits — and every branch sum is compared against the same circuit run directly, exactly, with no tolerance. Every T doubles the branches and the qubit count never enters: three T gates on ONE qubit are eight branches, six Clifford gates on five qubits are one. Depth is measured separately from gate count, in both directions (four gates at depth 1, four at depth 4). It does NOT make anything faster and says so: these branches run on the exact state vector at 2^t * 2^n, and realising the gain needs tableau branches with amplitude extraction and the ~2^0.23t decompositions, neither implemented. Falsified by a Clifford circuit the tableau cannot run in polynomial time, by a branch sum disagreeing with direct simulation (which aborts the run), or by a disagreement with the state vector: the stabilizer suite checks 200 random Clifford circuits in BOTH directions, requiring determined and undetermined outcomes to both appear.',
  ...
  'assert:tautology:check': 'reads every test under src/ for an assertion that compares an expression to itself AFTER a mutation in the same body. A determinism check (one expression twice, nothing between) is the survey and is not a gap; the same text after writeFileSync (or the other named mutators) is, because both sides evaluate after the write and no value from before survives to be compared. Ported from uuidna, where a receipt test shipped assert.equal(f(c)[k], f(c)[k], "stable") after a write it meant to be unmoved by, and a paren-blind first draft reported 0 over hundreds of files. This repository already gates X >= 0 via tautology; that matcher cannot see this shape. Falsified by planting the control dead line in a clone, which it reports by file:line; a determinism check without a mutation must NOT fail, and an unreadable test file fails as UNMEASURED rather than counting as a file with nothing wrong. The known limit is held in the predicate self-test: a determinism check after an unrelated fixture write is named, because narrowing to "the mutation target appears in the assertion" also clears the real dead line.',
