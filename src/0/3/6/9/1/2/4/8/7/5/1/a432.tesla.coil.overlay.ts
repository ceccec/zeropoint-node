...
import { TESLA_SEQUENCE, digitAngleToCMYK, cmykToCss, teslaPolarity } from './a432.math';
 ...
 */
export function logTeslaCoilEvent(blockchain: any, node: number, view: '2D' | '3D' | 'analytic', context: any) {
  const polarity = teslaPolarity(node);
  const spin = polarity;
  blockchain?.recordEvent?.(blockchain, 'teslaCoilOverlay', 'A432UI', { node, view, polarity, spin, ...context, timestamp: Date.now() });
} 