...
| Layer | Claim |
| --- | --- |
| **Exact** | fold, merge, merkle, digital root, vortex doubling mod 9, `computeVortexInvariantsHold()` |
| **Faithful** | torus / merkaba / animation / string·worldsheet (`foldStringTheory`) as projections of folds |
| **Refused** | Clay solved, Payload/ERP port, hand-minted ids |

`vortexInvariantsHold` computes `true|false` from **README gateway** seals (living field ∧ 60° dash closes incl. fusionIgnites ∧ developmentVortex ∧ KERNEL_SEALED). False ⇒ self-develop tips **quantumisation**.
