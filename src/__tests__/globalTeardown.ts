...
export default async function globalTeardown(): Promise<void> {
  ...
  try {
    ...
    const timers = activeTimers.filter(
      (handle: any) =>
    ...
    if (timers.length > 0) {
      console.log(
        `  - Found ${timers.length} remaining timers, attempting to clear...`,
      );
    ...
    const watchers = activeTimers.filter(
      (handle: any) => handle.constructor.name === "FSWatcher",
    );

    if (watchers.length > 0) {
      console.log(
        `  - Found ${watchers.length} remaining file watchers, closing...`,
      );
    ...
    console.log(
      "✅ All voids have been closed. The test field is unified and complete.",
    );
  } catch (error) {
