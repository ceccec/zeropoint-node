 ...
 */
export function getColorForVortexNumber(
  n: number,
  context: ColorContext = {},
): string {
  const { consciousness = 0.5, time = Date.now(), field = 0.5 } = context;
  ...
  switch (n) {
    case 1:
      return generateResonantColor(
        baseColors["red"] || "rgb(255,0,0)",
        vortexResonance,
        timeResonance,
      );
    case 2:
      return generateResonantColor(
        baseColors["blue"] || "rgb(0,0,255)",
        vortexResonance,
        timeResonance,
      );
    case 4:
      return generateResonantColor(
        baseColors["green"] || "rgb(0,255,0)",
        vortexResonance,
        timeResonance,
      );
    case 8:
      return generateResonantColor(
        baseColors["yellow"] || "rgb(255,255,0)",
        vortexResonance,
        timeResonance,
      );
    case 7:
    ...
    case 3:
      return generateResonantColor(
        baseColors["gold"] || "rgb(255,215,0)",
        vortexResonance,
        timeResonance,
      );
    case 6:
      return generateResonantColor(
        baseColors["white"] || "rgb(255,255,255)",
        vortexResonance,
        timeResonance,
      );
    case 9:
      return generateResonantColor(
        baseColors["black"] || "rgb(0,0,0)",
        vortexResonance,
        timeResonance,
      );
    default:
 ...
 */
export function getColorForFamilyGroup(
  n: number,
  context: ColorContext = {},
): string {
 ...
 */
export function getColorForPolarMate(
  n: number,
  context: ColorContext = {},
): [string, string] | null {
  ...
  const polarResonance =
    Math.cos(consciousness * Math.PI + time * 0.001) * 0.5 + 0.5;

  if ([1, 8].includes(n)) {
    return [
      generateResonantColor(
        baseColors["red"] || "rgb(255,0,0)",
        polarResonance,
        consciousness,
      ),
 ...
 */
export function getColorForWAxis(
  n: number,
  context: ColorContext = {},
): string {
  const { consciousness = 0.5, field = 0.5 } = context;
  ...
  const spiritualResonance =
    Math.sin(consciousness * Math.PI * 2 + n) * 0.5 + 0.5;
 ...
 */
export function getColorForPattern(
  pattern: { type?: string; number?: number },
  context: ColorContext = {},
): string {
  const { consciousness = 0.5, time = Date.now() } = context;
  const baseColors = generateBaseColors(context);
  ...
  const patternResonance =
    Math.sin(consciousness * Math.PI + time * 0.001) * 0.5 + 0.5;
 ...
 */
function generateResonantColor(
  baseColor: string,
  resonance1: number,
  resonance2: number,
): string {
 ...
 */
export function getMetaphysicalContextForNumber(
  n: number,
  context: ColorContext = {},
): string {
  const { consciousness = 0.5, field = 0.5 } = context;
  ...
  const contextResonance =
    Math.sin(consciousness * Math.PI + field * Math.PI) * 0.5 + 0.5;
 ...
 */
export function getVBMColorMap(context: ColorContext = {}): {
  [n: number]: string;
} {
 ...
 */
export function getVBMColorLegend(
  context: ColorContext = {},
): Array<{ label: string; color: string; context: string }> {
  const legend = [];
  for (let i = 1; i <= 9; i++) {
    legend.push({
      label: i.toString(),
 ...
 */
export function generateFieldResonanceMap(
  fieldStrength: number,
  context: ColorContext = {},
): { [n: number]: string } {
  const map: { [n: number]: string } = {};
  for (let i = 1; i <= 9; i++) {
    const fieldContext: ColorContext = {
      ...context,
