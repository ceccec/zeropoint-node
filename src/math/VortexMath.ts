...
export class VortexMath {
  ...
  public calculateResonance(level1: number = 0, level2: number = 0): number {
    ...
    const result = MathUtils.calculate('multiply', baseResonance, consciousnessMod);
  ...
  public calculateVortexEfficiency(value: number): number {
    const baseEfficiency = MathUtils.calculate('divide', value, MathUtils.calculate('add', 1, value));
    const fieldMod = MathUtils.calculate('multiply', this.fieldIntensity, MathUtils.calculate('vortex', value));
    
    const result = MathUtils.calculate('multiply', baseEfficiency, MathUtils.calculate('add', 1, fieldMod));
   ...
   */
  public calculateToroidalFlow(radius: number, angularVelocity: number): number {
    const vortexModulation = this.calculateVortexModulation(radius);
    
    const result = MathUtils.calculate('toroidal', radius, angularVelocity) * vortexModulation;
  ...
  public setVortexStrength(strength: number): void {
    this.vortexStrength = MathUtils.calculate('max', 0, MathUtils.calculate('min', 2, strength));
  }
  ...
  public setFieldIntensity(intensity: number): void {
    this.fieldIntensity = MathUtils.calculate('max', 0, MathUtils.calculate('min', 1, intensity));
  }
  ...
  private calculateVortexModulation(frequency: number): number {
    const baseModulation = MathUtils.calculate('sin', MathUtils.calculate('multiply', frequency, MATH_CONSTANTS.GOLDEN_RATIO));
    ...
    return MathUtils.calculate('add', 1, 
      MathUtils.calculate('multiply', baseModulation, 
        MathUtils.calculate('multiply', sequenceModulation, this.fieldIntensity)));
  }
   ...
   */
  public generateVortexFieldCoordinates(count: number, radius: number = 1): Array<[number, number, number]> {
    const coordinates: Array<[number, number, number]> = [];
    
    for (let i = 0; i < count; i++) {
      const angle = MathUtils.calculate('multiply', i, MathUtils.calculate('divide', 2 * Math.PI, count));
      const vortexRadius = MathUtils.calculate('multiply', radius, MathUtils.calculate('vortex', i));
      
      const x = MathUtils.calculate('multiply', vortexRadius, MathUtils.calculate('cos', angle));
      const y = MathUtils.calculate('multiply', vortexRadius, MathUtils.calculate('sin', angle));
  ...
  public calculateVortexField(x: number, y: number, z: number): number {
    const distance = MathUtils.calculate('sqrt', 
    ...
    const fieldStrength = MathUtils.calculate('multiply', 
  ...
  public generateVortexPattern(intensity: number): number[] {
    ...
    for (let i = 0; i < sequenceLength; i++) {
      ...
      const intensityMod = MathUtils.calculate('multiply', baseValue, intensity);
      const consciousnessMod = MathUtils.calculate('consciousness', i);
      
      const patternValue = MathUtils.calculate('multiply', intensityMod, consciousnessMod);
  ...
  public getColorForNumber(n: number = 1, _context?: any): string {
    // Simple stub for test compatibility
    const r = (n * 30) % 256;
    const g = (n * 60) % 256;
    const b = (n * 90) % 256;
...
export const applyVortexTransform = (input: number) => VortexMath.getInstance().applyVortexTransform(input); 