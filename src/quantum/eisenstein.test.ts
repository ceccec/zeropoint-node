...
const checker = createChecker('eisenstein')


// ── ω² = ω − 1 is what makes the ring integral ──────────────────────────────
checker.check('omega squared is omega minus one',
...
// ── closure: no product of ring elements leaves the ring ────────────────────
{
  ...
  checker.check(`every product of ${checked} pairs stays in the integer ring`, allIntegral, true)
}
...
for (const [a, b] of [[1, 0], [5, -3], [0, 1], [-7, 11], [1000000, -999999]]) {
  ...
  checker.check(`six 60-degree rotations return ${a}+${b}w exactly`, eisensteinEquals(z, start), true)
}

// A third of a turn is NOT the identity — otherwise the check above would pass
// for any function that did nothing at all.
checker.check('two rotations are not the identity', eisensteinEquals(rotate60(rotate60(EISENSTEIN_ONE)), EISENSTEIN_ONE), false)
checker.check('three rotations are not the identity',
...
// ── the six units, and there are exactly six ────────────────────────────────
{
  ...
  checker.check('there are six sixth roots of unity', units.length, 6)
  checker.check('every one has norm exactly 1', units.map((u) => eisensteinNorm(u)), [1n, 1n, 1n, 1n, 1n, 1n])
  checker.check('they are distinct', new Set(units.map((u) => `${u.a},${u.b}`)).size, 6)
}
...
for (const [a, b, c, d] of [[3, 4, -2, 5], [1, 1, 1, 1], [0, 7, 7, 0], [-9, 2, 4, -6]]) {
  ...
  checker.check(`N is multiplicative on (${a}+${b}w)(${c}+${d}w)`,
    eisensteinNorm(eisensteinMul(x, y)), eisensteinNorm(x) * eisensteinNorm(y))
  checker.check(`z times its conjugate is N(z) with zero w part for ${a}+${b}w`,
...
for (const [a, b, c, d] of [[3, 4, -2, 5], [0, 0, 1, 1], [-9, 2, 9, -2]]) {
  const x = eisenstein(a, b)
  const y = eisenstein(c, d)
  checker.check(`(x + y) - y is x for ${a}+${b}w`, eisensteinSub(eisensteinAdd(x, y), y), x)
  checker.check(`x - x is exactly zero for ${a}+${b}w`, eisensteinSub(x, x), EISENSTEIN_ZERO)
}

// ── the angle is 60° because the orbit is a six-cycle ───────────────────────
{
  const orbit = doublingOrbit()
  checker.check('the doubling orbit mod 9 has six elements', orbit.length, 6)
  checker.check('it is 1,2,4,8,7,5', orbit, [1, 2, 4, 8, 7, 5])
  checker.check('it closes back onto 1', (2 * orbit[orbit.length - 1]!) % 9, 1)
}

// ── π in the unit a 60° rotation defines ────────────────────────────────────
checker.check('pi is exactly three sixth-turns', piInSixthTurns(), 3n)
checker.check('and it is a count, not a rounded float', typeof piInSixthTurns(), 'bigint')
...
checker.check('no export of this module returns a decimal', [
