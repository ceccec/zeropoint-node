...
const checker = createChecker('exact')
...
for (const n of [2, 4, 10]) {
  ...
  checker.check(`GHZ_${n} first outcome is exactly 1/2`, frac(ps[0]!), '1/2')
  checker.check(`GHZ_${n} last outcome is exactly 1/2`, frac(ps[ps.length - 1]!), '1/2')
  checker.check(`GHZ_${n} everything else is exactly 0`, ps.slice(1, -1).every((p) => p.numerator === 0n), true)
  checker.check(`GHZ_${n} sums to exactly one`, exactlyNormalised(r), true)
}
...
// The float simulator does NOT, and that is the point of this module existing.
{
  ...
  const half = 1 / 2
  checker.check('the float simulator is NOT exactly 1/2 here', fl[0] === half, false)
  checker.check('the exact simulator IS', asFloat(exactProbability(ghzExact(10), 0)) === half, true)
}

// ── the gates, against facts that hold without any tolerance ────────────────
checker.check('H twice is the identity', frac(exactProbability(exactH(exactH(exactZeroState(1), 0), 0), 0)), '1/1')
checker.check('X moves all of it to |1>', frac(exactProbability(exactX(exactZeroState(1), 0), 1)), '1/1')
checker.check('Z alone does not move probability', frac(exactProbability(exactZ(exactZeroState(1), 0), 0)), '1/1')
checker.check('S alone does not move probability', frac(exactProbability(exactS(exactZeroState(1), 0), 0)), '1/1')
checker.check('Y takes |0> to |1>', frac(exactProbability(exactY(exactZeroState(1), 0), 1)), '1/1')
checker.check('X twice is the identity', frac(exactProbability(exactX(exactX(exactZeroState(1), 0), 0), 0)), '1/1')
{
  ...
  checker.check('CZ is symmetric in its qubits', ab, ba)
  ...
  checker.check('CNOT is not, on an input that can tell', na === nb, false)
  checker.check('CNOT with control 0 entangles |+0>', na, '1/2 0/1 0/1 1/2')
  checker.check('CNOT with control 1 leaves |+0> alone', nb, '1/2 1/2 0/1 0/1')
  checker.check('a superposition of two qubits is four quarters', exactProbabilities(plus).map(frac).join(' '), '1/4 1/4 1/4 1/4')
}
...
// ── agreement with the float simulator, within ITS tolerance ────────────────
{
  ...
  checker.check('exact and float agree to 1e-9 on a mixed circuit',
    fp.every((p, i) => { const d = p - ep[i]!; return d < 1e-9 && -d < 1e-9 }), true)
}

// ── the boundary is declared, not approximated ──────────────────────────────
checker.check('a non-Clifford gate is absent rather than approximated',
