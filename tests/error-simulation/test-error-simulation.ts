...
import { GoldenRatioCache } from './dist/src/__tests__/utils/goldenRatioCache';
import { ErrorSimulator } from './dist/src/__tests__/utils/errorSimulator';
...
for (let i = 1; i <= 10; i++) {
  try {
    ...
    console.log(`   Test ${i}: ❌ Error - ${(error as Error).message}`);
  }
}
