...
export function generateComponentCSS(component: string): string {
  const components = {
    button: `
.button {
...
export function calculateOpacity(level: number): string {
  const opacityMap = {
...
export function calculateScale(factor: number): string {
  const scaleMap = {
...
export function calculateWidth(percentage: number): string {
  const widthMap = {
